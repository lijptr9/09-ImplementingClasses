...
########################################################################
class Point(object):
    """represetns a point in 2d-space"""
    ...
    def move_to(self, x, y):
        self.distancetraveled = self.distancetraveled + math.sqrt((self.x - x) ** 2 + (self.y - y) ** 2)
        self.x = x
        self.y = y
        self.moves = self.moves + 1

    def move_by(self, dx, dy):
        self.distancetraveled = self.distancetraveled + math.sqrt(dx ** 2 + dy ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def closer_to(self, point2, point3):
        d1 = self.get_distance_from(point2)
        d2 = self.get_distance_from(point3)
        if d1 <= d2:
            return point2
        return point3
